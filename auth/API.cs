...
namespace Auth.GG_Winform_Example
{
    ...
    internal class OnProgramStart
    {
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            {
                MessageBox.Show("Invalid application information!", Name);
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show("Possible malicious activity detected. Please restart the program and try again.", "Authentication", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                        ...
                        case "success":
                            ...
                            else
                            {
                                if (response[12] == "Enabled")
                                {
                                    ...
                                    {
                                        MessageBox.Show($"File has been tampered with, couldn't verify integrity!", "Authentication");
                                        Process.GetCurrentProcess().Kill();
                                    }
                                }
                            ...
                            {
                                MessageBox.Show("The creator has locked this malware sample. This may be because the creator has initiated an emergency lockdown on all samples of this malware. Please contact the creator for more details.", "Authentication", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                Process.GetCurrentProcess().Kill();
                            }
                            break;
                        case "binderror":
    ...
    internal class API
    {
        ...
        public static bool AIOLogin(string AIO)
        {
            ...
            {
                MessageBox.Show("The authorisation key field cannot be empty.", "Authentication", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    if (Security.MaliciousCheck(response[1]))
                    {
                        MessageBox.Show("Possible malicious activity detected. Please restart the application and try again.", "Authentication", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Constants.Breached)
                    {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_details":
                            ...
                            return false;
                        case "time_expired":
                            MessageBox.Show("Your key has expired. Please request for an extension or for a new key.", "Authentication", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            return false;
                        case "hwid_updated":
        ...
        public static void Start()
        {
            ...
            else
            {
                using (StreamReader sr = new StreamReader($@"{drive}Windows\System32\drivers\etc\hosts"))
                {
                    ...
                    {
                        Constants.Breached = true;
                        MessageBox.Show("DNS redirecting has been detected. Heh, nice try.", "Authentication", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                }
